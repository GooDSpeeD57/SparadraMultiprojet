...
public class MedicamentDAO extends AbstractDAO<Medicament> {
    ...
    protected PreparedStatement prepareInsert(Medicament med, Connection conn) throws Exception {
        String sql = "INSERT INTO Medicament " +
                "(nomMedicament, categorie, prix, dateCirculation, stock, forme, sansOrdonnance) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        pst.setString(6, med.getFormeMedicament());
    ...
    // Retrait de stock
    public int retirerDuStock(int idMedicament, int quantite) throws Exception {
        ...
        if (quantite > med.getQuantiteMedicament())
            throw new SaisieException("Stock insuffisant !");
