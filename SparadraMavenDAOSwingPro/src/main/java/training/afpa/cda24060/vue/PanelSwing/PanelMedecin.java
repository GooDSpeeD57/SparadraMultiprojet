...
public class PanelMedecin extends JPanel {
    ...
    public PanelMedecin() throws SQLException {
        initComponents();
        chargerMedecins();
    }
    ...
    private JPanel creerPanelRecherche() {
        ...
        btnRechercheNom.addActionListener(e -> {
            try {
                rechercherParNom(txtRechercheNom.getText().trim());
            } catch (SQLException ex) {
                throw new RuntimeException(ex);
            }
        });
        ...
        btnRechercheRPPS.addActionListener(e -> {
            try {
                rechercherParRPPS(txtRechercheRPPS.getText().trim());
            } catch (SQLException ex) {
                throw new RuntimeException(ex);
            }
        });
    ...
    private void supprimerMedecin() throws SQLException {
        if (medecinIdSelectionne == -1) {
    ...
    public void chargerMedecins() throws SQLException {
    ...
    private void chargerMedecinDansFormulaire() throws SQLException {
    ...
    private void rechercherParRPPS(String rpps) throws SQLException {
        if (rpps.isEmpty()) {
            chargerMedecins();
            return;
        }
